      ...
      {selectedChat?.doc?.name ? (
        ...
        <>
          ...
          >
            <Button
              variant={'outline'}
              className='group max-w-48 cursor-pointer rounded-3xl border border-[#575757] bg-transparent px-5 text-xs text-[#575757] transition duration-300 hover:border-green-500 hover:bg-transparent dark:border-white dark:text-white dark:hover:border-mintGreen'
              onClick={() => setIsUploadDocOpen(true)}
            >
              <div className='flex items-center gap-2'>
                <Paperclip className='h-4 w-4 transition duration-300 group-hover:text-green-500 dark:text-[#EBEBEB] dark:group-hover:text-mintGreen' />
                <span className='font-semibold transition duration-300 group-hover:text-green-500 dark:group-hover:text-mintGreen'>
                  Attach Knowledge
                </span>
              </div>
            </Button>
